...
// ✅ Lấy danh sách sản phẩm trong đơn hàng
    ...
    public List<OrderItem> getOrderItemsByOrderId(long orderId, Connection conn) throws SQLException {
        List<OrderItem> list = new ArrayList<>();
        // Lỗi trong DB: bảng order_items thiếu cột product_name, discount
        // Tạm thời bỏ qua 2 cột đó
        String sql = "SELECT id, order_id, product_id, quantity, unit_price, total_price "
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    item.setUnitPrice(rs.getBigDecimal("unit_price"));
                    item.setTotalPrice(rs.getBigDecimal("total_price"));
                    // item.setProductName(rs.getString("product_name")); // Bảng của bạn không có cột này
                    // item.setDiscount(rs.getBigDecimal("discount")); // Bảng của bạn không có cột này
                    list.add(item);
                }
            }
        }
    ...
    public boolean updateOrderAndPaymentStatus(long orderId, String orderStatus, String paymentStatus) {
        String sql = "UPDATE orders SET status = ?, payment_status = ?, updated_at = NOW() WHERE id = ?";
